...
> **Данни за `/conflicts` (свързани лица, #287) → `sigma-279`, не `sigma-dev`.** Изгледът гейтва всяка
> връзка на наличие на ред в `interest_link_evidence` (доказателството от Търговския регистър, #279 /
> ADR-0033). Базата `sigma-dev` **няма** тази таблица, затова срещу нея `/conflicts` рендира празното
> състояние. Затова `SIGMA_D1_ID` / `SIGMA_D1_NAME` на `preview` средата сочат към **`sigma-279`**
> (`b7edd1d1-…`) — база с пълната #279 схема и данни (692 доказателства, ~337 показани връзки). Останалите
> страници работят и от двете (`sigma-279` е надмножество на `sigma-dev`). Ако `sigma-279` бъде изтрита,
> върни ги към dev базата **след** като заредиш `interest_link_evidence` в нея. Гардът в `preview.yml`
> отказва само продукционното `SIGMA_D1_NAME=sigma`; `sigma-279` минава.
